...
const MODELS_PAGE_FAQS = [
  ...
  },
  {
    ...
    answer:
      'The Tesla Model S Long Range offers the highest EPA-rated range in the Tesla lineup, exceeding 640 km on a single charge. The Model 3 Long Range and Model Y Long Range also offer excellent range above 480 km.',
  },
  {
  ...
  },
  {
    ...
    answer:
      'The Tesla Model Y is the most popular choice for families thanks to its spacious interior, large cargo area (2,150 L with seats folded), and optional third-row seating for 7 passengers. The Model X offers even more space plus falcon-wing doors for easy child-seat access.',
  },
  {
  ...
  return (
    <main className="mx-auto max-w-[1400px] px-8 py-12">
      ...
      <header className="mb-12 text-center">
        <span className="font-mono text-[13px] font-medium uppercase tracking-[1.5px] text-ink-3">
          Complete Lineup
        </span>
      ...
      <section className="mt-20">
        <span className="mb-2 block text-center font-mono text-[13px] font-medium uppercase tracking-[1.5px] text-ink-3">
          FAQ
        </span>
        <h2 className="mb-8 text-center font-display text-[30px] font-bold tracking-[-1.5px] text-foreground sm:text-[38px]">
        ...
        <div className="mx-auto max-w-3xl space-y-4">
          {MODELS_PAGE_FAQS.map((faq) => (
            ...
            >
              <summary className="flex cursor-pointer items-center justify-between p-6 text-[16px] font-medium text-foreground [&::-webkit-details-marker]:hidden">
                <span>{faq.question}</span>
                <span className="ml-4 text-[20px] text-ink-3 transition-transform group-open:rotate-45">
                  +
                </span>
              </summary>
              <div className="px-6 pb-6 text-[16px] leading-relaxed text-ink-2">
                {faq.answer}
              </div>
            </details>
          ))}
